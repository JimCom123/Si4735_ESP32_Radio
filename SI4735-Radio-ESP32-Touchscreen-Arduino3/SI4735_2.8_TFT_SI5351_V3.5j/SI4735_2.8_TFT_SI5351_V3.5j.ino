...
const char *Rel_Vers[]   =    { "3.5j"};
#define ESP32_I2C_SDA    21  // I2C bus pin on ESP32
#define ESP32_I2C_SCL    22  // I2C bus pin on ESP32
#define RESET_PIN        12
#define ENCODER_PIN_A    16  //17  // http://www.buxtronix.net/2011/10/rotary-encoders-done-properly.html
#define ENCODER_PIN_B    17  //16
...
#define MIN_ELAPSED_DISPL_TIME      2000  ////
#define MIN_ELAPSED_RDS_TIME           5
#define DEFAULT_VOLUME               100  //// 45   // change it for your favorite start sound volume
...
Button bt[] = {        //                                                 | 11 |
  ...
  { "BFO"   , 11 , "SEEKDN",  4 , Xbut1 , Ybut1  }, //     |  0 |  1 |  2 |  3 |
  { "FREQ"  ,  2 , "STATUS",  6 , Xbut2 , Ybut2  }, //     |----|----|----|----|
  { "AGC"   ,  4 , "RDS"   ,  8 , Xbut3 , Ybut3  }, //     |  4 |  5 |  6 |  7 |
  { "MUTE"  ,  8 , "AGCset",  1 , Xbut4 , Ybut4  }, //     |----|----|----|----|
  { "VOL"   ,  9 , ""      , 10 , Xbut5 , Ybut5  },
  { "MODE"  ,  3 , "DISPL" ,  2 , Xbut6 , Ybut6  },
  { "FILTER",  5 , ""      , 11 , Xbut7 , Ybut7  },
  { "STEP"  ,  6 , "CHIP"  ,  3 , Xbut8 , Ybut8  },
  { "BAND"  ,  1 , ""      ,  5 , Xbut9 , Ybut9  },
...
Bandwidth bw[] = {                                 //  AM    SSB   FM
  { 4 , 4 , 0 , XfBW, 100, YfBW, 30,   0},         //  1.0   0.5   Auto
...
//   Band table
Band band[] = {
#ifdef JapanAsiabandplan
  {   "FM", FM_BAND_TYPE,  FM,  6400, 10800,  8810, 10, 0}, //  FM          0
  {   "LW", LW_BAND_TYPE,  AM,   130,   279,   164,  1, 0}, //  LW          1
  {   "MW", MW_BAND_TYPE,  AM,   522,  1701,   666,  9, 0}, //  MW          2
  {"BACON", LW_BAND_TYPE,  AM,   280,   470,   284,  1, 0}, // Ham          3
  { "630M", LW_BAND_TYPE, LSB,   472,   479,   475,  1, 0}, // Ham  630M    4
  { "160M", SW_BAND_TYPE, LSB,  1800,  1920,  1910,  1, 0}, // Ham  160M    5
  { "120M", SW_BAND_TYPE,  AM,  2300,  2900,  2400,  5, 0}, //      120M    6
  {  "90M", SW_BAND_TYPE,  AM,  3200,  3400,  3250,  5, 0}, //       90M    7
  {  "80M", SW_BAND_TYPE, LSB,  3500,  3810,  3540,  1, 0}, // Ham   80M    8
  {  "75M", SW_BAND_TYPE,  AM,  3900,  4000,  3925,  5, 0}, //       75M    9
  {  "60M", SW_BAND_TYPE, USB,  5330,  5410,  5375,  1, 0}, // Ham   60M   10
  {  "49M", SW_BAND_TYPE,  AM,  5900,  6200,  6055,  5, 0}, //       49M   11
  {  "40M", SW_BAND_TYPE, LSB,  7000,  7200,  7100,  1, 0}, // Ham   40M   12
  {  "41M", SW_BAND_TYPE,  AM,  7200,  7600,  7580,  5, 0}, //       41M   13
  {  "31M", SW_BAND_TYPE,  AM,  9400,  9900,  9665,  5, 0}, //       31M   14
  {  "30M", SW_BAND_TYPE, USB, 10100, 10150, 10120,  1, 0}, // Ham   30M   15
  {  "25M", SW_BAND_TYPE,  AM, 11600, 12100, 11680,  5, 0}, //       25M   16
  {  "22M", SW_BAND_TYPE,  AM, 13570, 13870, 13700,  5, 0}, //       22M   17
  {  "20M", SW_BAND_TYPE, USB, 14000, 14350, 14250,  1, 0}, // Ham   20M   18
  {  "19M", SW_BAND_TYPE,  AM, 15100, 15830, 15280,  5, 0}, //       19M   19
  ...
  {  "15M", SW_BAND_TYPE, USB, 21000, 21450, 21250,  1, 0}, // Ham   15M   23
  ...
  {   "CB", SW_BAND_TYPE,  AM, 26200, 27995, 27000,  1, 0}, // CB band     27
  {  "10M", SW_BAND_TYPE, USB, 28000, 30000, 28500,  1, 0}, // Ham   10M   28
  {   "SW", SW_BAND_TYPE,  AM,  1730, 30000, 10000,  5, 0}  // Whole SW    29
...
////  uint16_t calData[5] = { 387, 3530, 246, 3555, 7 };
  uint16_t calData[5] = { 444, 3393, 352, 3330, 7 };
  tft.setTouch(calData);
#endif
...
////  delay(500);
  ...
  si4735.setSeekFmLimits(7600, 10800);
  ...
  si4735.setFMDeEmphasis(1);  //Set FM de-emphasis to 50us
  si4735.setFmBlendRssiStereoThreshold(38); // 49dBuV = 0x0031
  si4735.setFmBLendRssiMonoThreshold(20); // 30dBuV = 0x001e
  si4735.setFmBlendSnrStereoThreshold(26); // default 26dB
  si4735.setFmBLendSnrMonoThreshold(12); // default 12dB
          ...
          if ((x > ((kp[n].Xkeypos) + (kp[n].Xkeypnr))) and (x < ((kp[n].Xkeypos) + (kp[n].Xkeypsr) + (kp[n].Xkeypnr))) and (y > ((kp[n].Ykeypos) + (kp[n].Ykeypnr))) and (y < ((kp[n].Ykeypos) + (kp[n].Ykeypsr) + (kp[n].Ykeypnr)))) {
            ...
            if (n == 11) {// SET button
              ...
              if (Displayfreq < 1) {
                ...
                if ((Displayfreq > 30) and (Displayfreq < 64.0 )) {
                  tft.setCursor(7, 25);
                  tft.print("Freqency > 30MHz and < 64 MHz");
                  ErrorBeep();
                } else {
                  if ((Displayfreq >= 108) and (Displayfreq < 130 )) {
                    tft.setCursor(7, 25);
                    tft.print("Freqency >= 108 and < 130 ");
                    ErrorBeep();
                  } else {
                    if (Displayfreq > 30000) Displayfreq = Displayfreq / 1000000;
                    if ((Displayfreq <= 30000) and (Displayfreq >= 130) and (Decipoint == false )) Displayfreq = Displayfreq / 1000;
                    if ((Displayfreq >= 64.0) and (Displayfreq <= 108.0)) {
...
uint8_t t1, t2, t3, t4, max;
  ...
  if ((millis() - elapsedRSSI) > MIN_ELAPSED_RSSI_TIME * RSSIfact) // 150 * 1  = 150 msec refresh time RSSI
  {
    ...
    NewCap = si4735.getAntennaTuningCapacitor();
...
////    tft.fillRect(XFreqDispl + 7, YFreqDispl + 75 , 173, 10, TFT_BLACK);
    tft.fillRect(XFreqDispl + 7, YFreqDispl + 75 , 173, 18, TFT_BLACK);
...
////    tft.drawString("SNR = " + String(NewSNR) + " dB", XFreqDispl + 180, YFreqDispl + 76);
    ...
    tft.setTextDatum(TL_DATUM); ////
    tft.drawString("Cap = " + String(NewCap) , XFreqDispl + 8, YFreqDispl + 83); ////
  }
  VOLbutoff();
}
...
////    tft.fillRect(Xsmtr + 15 + (i * 12), Ysmtr + 24, 4, 8, TFT_YELLOW);
    tft.fillRect(Xsmtr + 15 + (i * 12), Ysmtr + 24, 2, 4, TFT_YELLOW);
    IStr = String(i);
...
////    tft.fillRect((Xsmtr + 123 + (i * 16)), Ysmtr + 24, 4, 8, TFT_RED);
    tft.fillRect((Xsmtr + 123 + (i * 16)), Ysmtr + 24, 2, 4, TFT_RED);
    IStr = String(i * 10);
...
////  tft.fillRect(Xsmtr + 127, Ysmtr + 32 , 100, 4, TFT_RED);
  tft.fillRect(Xsmtr + 15, Ysmtr + 28 , 112, 2, TFT_YELLOW);
  tft.fillRect(Xsmtr + 127, Ysmtr + 28 , 100, 2, TFT_RED);
  // end Smeter
...
////  tft.fillRect(XFreqDispl + 5, YFreqDispl + 5, 230, 80, TFT_BLACK);
    ...
    tft.setTextSize(1);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Mode", XFreqDispl + 80, YFreqDispl + 8);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
...
void BFOStepdisp() {
  ...
  if (band[bandIdx].bandType != FM_BAND_TYPE) {
    ...
    if (bfoOn) {
      ...
      tft.setTextColor(TFT_GREEN, TFT_BLACK);
      tft.drawString("  ", XFreqDispl + 20, YFreqDispl + 18);  //26
    }
  }
}
